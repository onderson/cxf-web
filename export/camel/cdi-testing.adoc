...
|link:cdi-testing.html[Camel CDI Test] a|
* JUnit 4
...
|link:cdi-testing.html[Arquillian] a|
* JUnit 4
* TestNG 5
...
|link:cdi-testing.html[PAX Exam] a|
* JUnit 4
* TestNG 6
...
annotation provided by Camel CDI test. For example, if you need to add a
route to your Camel context, instead of declaring a `RouteBuilder`
...
You can find more information in link:cdi.html[auto-detecting Camel
routes].

In case you prefer declaring the `RouteBuilder` bean in a separate
class, for example to share it more easily across multiple test classes,
you can use the `@Beans` annotation to instruct Camel CDI test to deploy
that class as a CDI bean, e.g.:
...
behaviour of the application bean.
...
If you're using Arquillian as testing framework, you need to activate
the alternative in your deployment method, e.g.: 
...
can __advice__ an existing route before its being tested. It allows to
...
That way, you can use JUnit class rules to initialise (resp. clean-up)
resources that your test classes would require during their execution
before the container initialises (resp. after the container has
...
Another use case is to assert the behaviour of your application after it
