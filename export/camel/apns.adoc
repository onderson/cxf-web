...
|`tokens` |  |Empty by default. Configure this property in case you want
to statically declare tokens related to devices you want to notify.
...
|`userFixedDelay` |`true` |If `true`, use fixed delay between pools,
...
|`CamelApnsMessageType` |`STRING, PAYLOAD, APNS_NOTIFICATION`  |In case
you choose PAYLOAD for the message type, then the message will be
considered as a APNS payload and sent as is. In case you choose STRING,
message will be converted as a APNS payload. From *Camel 2.16* onwards
APNS_NOTIFICATION is used for sending message body as
