...
|shutdownGraceful |*Camel 2.11:* Marks the thread pool as shutdown, and
graceful shutdown the pool, by waiting for tasks to complete. A default
timeout value of 10 sec is used, before +
shutdown becomes aggressive using `shutdownNow`, to force threads to
shutdown.

|shutdownGraceful(timeout) |*Camel 2.11:* As above but with custom
timeout value

|awaitTermination |*Camel 2.11:* To wait graceful for the termination of
a thread pool (eg to wait for its tasks to complete). Will wait until
all tasks is completed or a timeout value is hit.
|=======================================================================
